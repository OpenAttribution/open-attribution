...
randomsuffix="$(LC_ALL=C tr -dc a-z0-9 </dev/urandom | head -c 4)"
...
if [ "$dolaunchsess" = true ]; then
    ...
    tmux send-keys -t $session "~/app-spy/proxysetup.sh $port -w $logfile" Enter
    # Bottom Right
    ...
    tmux send-keys -t $session "tail -f ${WAYDROID_LOG_FILE} | grep --line-buffered 'user 0 is ready' | while read ; do ~/app-spy/check_app.sh -s $app && echo 'Launch Waydroid UI..'. && waydroid app launch $app ; done" Enter
